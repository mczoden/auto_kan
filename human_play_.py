...
def random_mouse_move(random_range=40):
    ...
    random_range = min([
        ...
        x,
        y,
        game_gui['window']['x'] * 2 - x,
        game_gui['window']['y'] * 2 - y,
    ])
    if random_range < 0:
...
def confirm_areas(areas):
    for area in areas:
        current_color = pyautogui.pixel(
            area['x'] + game_gui['offset_x'],
            area['y'] + game_gui['offset_y'])
        if 'color_match_func' in area:
            if not area['color_match_func'](*current_color):
                return False
        else:
            match = False
            for color in area['colors']:
...
def wait_confirm_areas(areas, interval=0.5, step=0.5, timeout=float('inf')):
    if DEBUG_SKIP_WAIT:
        return True
    total_wait_time = 0
    if timeout < 0:
...
def wait_not_confirm_areas(areas, interval=0.5, step=0.5):
    if DEBUG_SKIP_WAIT:
        return True
    while True:
...
def sortie_from_port():
    ...
    if is_at_sortie_main():
        return
    button = game_gui['sortie_in_port']
    move_to_button(button)
    if not DEBUG_SKIP_WAIT:
...
def expedition_from_sortie():
    ...
    if is_at_expedition_in_sortie():
        return
    button = game_gui['expedition_in_sortie']
    move_to_button(button)
    if not DEBUG_SKIP_WAIT:
        time.sleep(random.random() * 0.3 + 0.2)
    human_mouse.click()
...
def select_expedition_map_bottom_label(e):
    expedition_button = game_gui['map{}_in_expedition'.format(e['map'])]
...
def get_current_expedition_fleet():
    ...
    for i in range(2, 5):
        areas_to_check = \
            game_gui['expedition_fleet{}_is_selected_confirm_areas'.format(i)]
...
def do_expedition(fleet):
    move_to_button(game_gui['confirm_expedition'])
    time.sleep(random.random() * 0.1 + 0.5)
    human_mouse.click()
    time.sleep(random.random() * 0.1 + 0.5)
    ...
    if fleet != get_current_expedition_fleet():
        ...
        move_to_button(game_gui['select_expedition_fleet{}'.format(fleet)])
        time.sleep(random.random() * 0.1 + 0.2)
        human_mouse.click()

    current_x, current_y = position_in_game()
    if not human_mouse.is_dot_in_area(
            current_x, current_y, game_gui['start_expedition']):
        move_to_button(game_gui['start_expedition'])
...
def check_all_expedition():
    ...
    move_to_button(game_gui['expedition_check'])
    ...
    while confirm_areas(game_gui['expedition_flag_confirm_areas']):
        ...
        time.sleep(random.random() * 0.5 + 0.2)
        debug('click check result')
        human_mouse.click()
        random_mouse_move()

        time.sleep(random.random() * 0.5 + 0.2)
        debug('click return to port')
        human_mouse.click()

        time.sleep(random.random())
        wait_confirm_areas(game_gui['port_confirm_areas'])
        time.sleep(random.random() * 0.5 + 0.2)
...
def select_and_do_expedition(e_no, fleet):
    select_expedition(e_no)
    time.sleep(random.random() * 0.1 + 0.5)
    do_expedition(fleet)


def select_and_do_expeditions(e_f):
...
def goto_supply_from_port():
    ...
    if not is_at_port():
        back_to_port_from_upper_left()
        time.sleep(random.random() * 0.5 + 0.5)
    button = game_gui['supply_in_port']
    move_to_button(button)
    if not DEBUG_SKIP_WAIT:
        time.sleep(random.random() * 0.2 + 0.2)
    human_mouse.click()
...
def select_fleet_to_supply(fleet):
...
def is_need_to_supply():
    return confirm_areas(game_gui['1st_ship_need_to_supply_confirm_areas'])
...
def supply(fleet):
    ...
    move_to_button(game_gui['supply_all_in_supply'])
    time.sleep(random.random() * 0.5 + 0.2)
    # pyautogui.confirm('click here')
    human_mouse.click()
    wait_not_confirm_areas(game_gui['1st_ship_need_to_supply_confirm_areas'])
